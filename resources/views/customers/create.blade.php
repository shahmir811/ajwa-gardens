      ...
      <div class="six wide tablet eight wide computer column">
        ...
        <form class="ui form" method="POST" action="{{ route('customers.store') }}">
          ...
          <div class="field @error('father_name') error @enderror">
              ...
              <input id="FatherName" type="text" name="father_name" placeholder="Name" value="{{ old('father_name') }}" autofocus>
              @error('father_name')
                <span class="field error">
                    <strong class="error-text">{{ $message }}</strong>
                </span>
          ...
          <div class="field @error('address') error @enderror">
              <label>Address</label>
              <textarea id="Name" rows=2 name="address" placeholder="Name">{{ old('address') }}</textarea>
              @error('address')
                <span class="field error">
                    <strong class="error-text">{{ $message }}</strong>
                </span>
