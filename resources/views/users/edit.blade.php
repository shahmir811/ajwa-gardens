...
@section('content')
    ...
    <h1 class="ui header">Edit User Details</h1>
      ...
      <div class="six wide tablet eight wide computer column">
        ...
        <form class="ui form" method="POST" action="{{ route('users.update',$user->id) }}">
          ...
          <div class="field @error('name') error @enderror">
              <label>Name</label>
              <input id="Name" type="text" name="name" placeholder="Name" value="{{ old('name', $user->name) }}" autofocus>
              @error('name')
          ...
          <div class="field">
            ...
            <select name="role_id" class="ui fluid dropdown" value="{{ old('role_id') }}">
              ...
              <option value="2" {{ ( old("role_id", $user->role_id) == 2 ? "selected":"") }}>Employee</option>
            </select>
          </div>
